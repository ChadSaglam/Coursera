...
DATA_URL = (
    "/Users/chadsglm/Documents/GitHub/AI_BootCamp/Assignments & Projects/Projects/Coursera/Create Interactive Dashboards with Streamlit and Python/Tweets.csv"
)
...
select = st.sidebar.selectbox('Visualization type', ['Bar plot', 'Pie chart'], key='viz_type_1')
sentiment_count = data['airline_sentiment'].value_counts()
sentiment_count = pd.DataFrame({'Sentiment':sentiment_count.index, 'Tweets':sentiment_count.values})
if not st.sidebar.checkbox("Hide", True, key='hide_1'):
...
if not st.sidebar.checkbox("Close", True, key='close_1'):
    st.markdown("### Tweet locations based on time of day")
    st.markdown("%i tweets between %i:00 and %i:00" % (len(modified_data), hour, (hour + 1) % 24))
    st.map(modified_data)
    if st.sidebar.checkbox("Show raw data", False, key='show_raw_1'):
        st.write(modified_data)

st.sidebar.subheader("Total number of tweets for each airline")
each_airline = st.sidebar.selectbox('Visualization type', ['Bar plot', 'Pie chart'], key='viz_type_2')
...
if not st.sidebar.checkbox("Close", True, key='close_2'):
...
if len(choice) > 0:
    ...
    breakdown_type = st.sidebar.selectbox('Visualization type', ['Pie chart', 'Bar plot', ], key='viz_type_3')
    fig_3 = make_subplots(rows=1, cols=len(choice), subplot_titles=choice)
    if breakdown_type == 'Bar plot':
        for i in range(1):
            for j, airline in enumerate(choice):
                fig_3.add_trace(
                    go.Bar(x=plot_sentiment(airline).Sentiment, y=plot_sentiment(airline).Tweets, showlegend=False),
                    row=i+1, col=j+1
                )
        ...
        for i in range(1):
            for j, airline in enumerate(choice):
                fig_3.add_trace(
                    go.Pie(labels=plot_sentiment(airline).Sentiment, values=plot_sentiment(airline).Tweets, showlegend=True),
                    i+1, j+1
                )
        fig_3.update_layout(height=600, width=800)
        st.plotly_chart(fig_3)
...
st.sidebar.subheader("Breakdown airline by sentiment")
choice = st.sidebar.multiselect('Pick airlines', ('US Airways','United','American','Southwest','Delta','Virgin America'), key='airline_choice')
if len(choice) > 0:
...
if not st.sidebar.checkbox("Close", True, key='close_3'):
